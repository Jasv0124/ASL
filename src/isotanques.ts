...
function palletsDashboard(rows:IsoRecord[]){
 ...
 return `<div class="iso-grid">${panel('Total estibas por CD',horizontal(centers),'wide')}${panel('Participación por CD',donut,'wide')}${panel('Total estibas por gerencia',funnel(management),'wide')}${panel('Tipo C por CD',typeChart,'wide')}${panel('Inventario A, B y C',abc,'span2')}${panel('Seguimiento diario',dailyTable(rows,'totalPallets'),'wide')}</div>`;
}
